...
namespace WL.Data.Context
{
    public class AppDbContext : DbContext
    {
        ...
        public void Seed()
        {
            ...
            if (!Users.Any() && !Wallets.Any())
            {
                // Criando usuários fictícios
                var user1 = new User("John Doe", "john.doe@example.com", "password123");
                var user2 = new User("Jane Smith", "jane.smith@example.com", "password456");

                // Adicionando os usuários ao DbSet
                Users.AddRange(user1, user2);

                // Salvando os usuários para gerar IDs
                SaveChanges();
                ...
                // Salvando as carteiras no banco
                SaveChanges();

                // Criando algumas transferências fictícias
                var transfer1 = new Transfer(user1.Id, user2.Id, DateTime.Now, 200.00m, wallet1.Id, wallet2.Id, "Jane Smith");
                var transfer2 = new Transfer(user2.Id, user1.Id, DateTime.Now.AddMinutes(10), 100.00m, wallet2.Id, wallet1.Id, "John Doe");
